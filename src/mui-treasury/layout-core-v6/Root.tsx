...
export function applyRootStyles(params?: { height?: string | Record<Breakpoint, string> | Record<string, string>; fixedHeight?: boolean }) {
...
const StyledRoot = styled('div')(({ theme }) => ({
  ...
  gridTemplateColumns: 'var(--_start-col, 0px) minmax(0, 1fr) var(--_end-col, 0px)', // minmax(0, 1fr) is used over `1fr` to prevent root horizontal overflow
  gridTemplateAreas: `
...
const Root = React.forwardRef<HTMLDivElement, BoxProps>(function Root({ className, ...props }, ref) {
  return <StyledRoot ref={ref} className={`${layoutClasses.Root} ${className || ''}`} {...props} />
})
